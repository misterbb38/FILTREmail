...
v4: MongoDB для хранения обработанных писем, dotenv, Render-ready.
...
def is_processed(uid_str: str) -> bool:
    return processed_col.find_one({"uid": uid_str}) is not None


def mark_processed(uid_str: str, sender: str, subject: str, is_order: bool, confidence: float, reason: str):
...
def run():
    ...
    try:
        ...
        uids = message_ids[0].split()
        if not uids:
            log.info("Нет новых писем за сегодня")
            return
        ...
        for uid, uid_str, msg in emails_to_process:
            ...
            # Сохраняем в MongoDB — больше не будет повторно обработано
            mark_processed(uid_str, sender, subject, is_order, confidence, reason)
            processed_count += 1
    ...
    log.info(f"\nИтого: обработано {processed_count}, заявок перемещено: {moved_count}")
    log.info(f"{'='*60}\n")
